...
export default function EditCarAdminPage({ params }) {
  ...
  const [formData, setFormData] = useState({
    ...
    licensePlate: "", ownerCCCD: "" // 🚀 THÊM 2 BIẾN MỚI
  });
    ...
    const fetchCar = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // 🚀 GIẢI MÃ AN TOÀN MẢNG / CHUỖI TỪ DATABASE
          let safeGallery = [];
          ...
          setFormData({
            ...
            licensePlate: data.licensePlate || "", // 🚀 LẤY BIỂN SỐ
            ownerCCCD: data.ownerCCCD || ""       // 🚀 LẤY CCCD
          });
        }
      } catch (error) {
        console.error(error);
  ...
  const handleSubmit = async (e: any) => {
    ...
    // 🚀 ÉP KIỂU DỮ LIỆU CHUẨN TRƯỚC KHI GỬI LÊN API
    const payload = {
      ...formData,
      ...
      // Chuyển chuỗi phân cách bằng dấu phẩy thành mảng Array xịn
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          <h1 className="text-2xl font-black text-blue-900 uppercase tracking-tighter flex items-center gap-3 mb-8">
            <CarFront className="text-blue-600" /> Cập nhật thông tin xe #{carId}
          </h1>

          <form onSubmit={handleSubmit} className="space-y-6">
            
            {/* 1. THÔNG TIN ĐỊNH DANH (MỚI THÊM) */}
            ...
            <button disabled={loading} type="submit" className="w-full py-5 bg-blue-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all flex justify-center items-center gap-2">
              <Save size={20} /> {loading ? "Đang lưu thay đổi..." : "Lưu cập nhật hồ sơ xe"}
            </button>
          </form>
